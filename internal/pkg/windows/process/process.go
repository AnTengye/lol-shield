...
func GetProcessFullPath(targetName string) (string, error) {
	...
	if err != nil {
		syslog.L.Debug("无法获取到进程handle: ", err)
		return "", errNotFoundProcess
	}
	buf := [MAX_PATH]uint16{}
	size := MAX_PATH
	ret, _, lastErr := queryFullProcessImageName.Call(
		uintptr(hProcess), 0,
...
func GetCmdline(pid uint32) (string, error) {
	...
	err = windows.ReadProcessMemory(
		h, uintptr(unsafe.Pointer(commandLine.Buffer)),
